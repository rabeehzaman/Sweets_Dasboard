-- ============================================================================
-- SWEETS DEPARTMENT DATABASE VIEW MIGRATION
-- Migrate critical views from MADA to SWEETS database
-- ============================================================================

-- IMPORTANT: Run these scripts in order as views have dependencies

-- ============================================================================
-- STEP 1: TABLE ALIGNMENT (RUN FIRST)
-- Rename existing SWEETS tables to match MADA naming convention
-- ============================================================================

-- Check if tables exist and rename them
DO $$
BEGIN
    -- Rename fifo_mapping_table to fifo_mapping
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'fifo_mapping_table' AND schemaname = 'public') THEN
        ALTER TABLE fifo_mapping_table RENAME TO fifo_mapping;
        RAISE NOTICE 'Renamed fifo_mapping_table to fifo_mapping';
    END IF;

    -- Rename stock_in_flow_table to stock_in_flow
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'stock_in_flow_table' AND schemaname = 'public') THEN
        ALTER TABLE stock_in_flow_table RENAME TO stock_in_flow;
        RAISE NOTICE 'Renamed stock_in_flow_table to stock_in_flow';
    END IF;

    -- Rename stock_out_flow_table to stock_out_flow
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'stock_out_flow_table' AND schemaname = 'public') THEN
        ALTER TABLE stock_out_flow_table RENAME TO stock_out_flow;
        RAISE NOTICE 'Renamed stock_out_flow_table to stock_out_flow';
    END IF;

    -- Rename bill_item to bill_items
    IF EXISTS (SELECT FROM pg_tables WHERE tablename = 'bill_item' AND schemaname = 'public') THEN
        ALTER TABLE bill_item RENAME TO bill_items;
        RAISE NOTICE 'Renamed bill_item to bill_items';
    END IF;
END $$;

-- ============================================================================
-- STEP 2: CREATE MISSING TABLES (SIMPLIFIED VERSIONS)
...
-- ============================================================================
-- STEP 3: CREATE CORE VIEW - profit_analysis_view_current
-- This is the main view that powers the dashboard
-- ============================================================================
...
SELECT
    ...
    -- Cost calculation from FIFO (simplified for now)
    COALESCE(SUM(CASE
...
LEFT JOIN stock_out_flow sof ON ii.item_id = sof.invoice_item_id
LEFT JOIN fifo_mapping fm ON sof.stock_out_flow_id = fm.stock_out_flow_id
LEFT JOIN stock_in_flow sif ON fm.stock_in_flow_id = sif.stock_in_flow_id
...
LEFT JOIN stock_in_flow sif ON cni.item_id = sif.credit_notes_item_id
...
-- ============================================================================
-- STEP 4: CREATE customer_balance_aging VIEW
-- Customer accounts receivable aging analysis
-- ============================================================================
...
-- No HAVING clause for SWEETS - show all customers
ORDER BY total_balance DESC;

-- ============================================================================
-- STEP 5: CREATE customer_balance_aging_filtered VIEW
-- No filtering needed for SWEETS - just alias to main view
-- ============================================================================
...
-- ============================================================================
-- STEP 6: CREATE vendor_bills_filtered VIEW
...
-- ============================================================================
-- STEP 7: CREATE top_overdue_customers VIEW
-- Show all customers with overdue calculations (no filtering for SWEETS)
-- ============================================================================
...
-- ============================================================================
-- STEP 9: CREATE RPC FUNCTIONS FOR OPTIMIZED QUERIES
-- These functions provide optimized access to aggregated data
-- ============================================================================
...
-- Revert table names (if needed)
ALTER TABLE fifo_mapping RENAME TO fifo_mapping_table;
ALTER TABLE stock_in_flow RENAME TO stock_in_flow_table;
ALTER TABLE stock_out_flow RENAME TO stock_out_flow_table;
ALTER TABLE bill_items RENAME TO bill_item;
*/
